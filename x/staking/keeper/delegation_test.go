...
func (s *KeeperTestSuite) TestDelegationsByValIndex() {
	...
	_, err = s.msgServer.Undelegate(ctx, stakingtypes.NewMsgUndelegate(addrDels[0].String(), valAddrs[0].String(), types.FlexibleDelegationID, sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(1))))
	require.NoError(err)
	...
	_, err = s.msgServer.Undelegate(ctx, stakingtypes.NewMsgUndelegate(addrDels[1].String(), valAddrs[0].String(), types.FlexibleDelegationID, sdk.NewCoin(sdk.DefaultBondDenom, math.NewInt(2))))
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUndelegateSelfDelegationBelowMinSelfDelegation() {
	...
	_, _, err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], types.FlexibleDelegationID, math.LegacyNewDecFromInt(keeper.TokensFromConsensusPower(ctx, 6)))
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUndelegateFromUnbondingValidator() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.BondedPoolName, stakingtypes.NotBondedPoolName, gomock.Any())
	_, amount, err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], types.FlexibleDelegationID, math.LegacyNewDecFromInt(delTokens))
	require.NoError(err)
	...
	_, undelegatedAmount, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], types.FlexibleDelegationID, undelegateAmount)
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUndelegateFromUnbondedValidator() {
	...
	// unbond the all self-delegation to put validator in unbonding state
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.BondedPoolName, stakingtypes.NotBondedPoolName, gomock.Any())
	_, amount, err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], types.FlexibleDelegationID, math.LegacyNewDecFromInt(valTokens))
	require.NoError(err)
	...
	_, amount2, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], types.FlexibleDelegationID, math.LegacyNewDecFromInt(unbondTokens))
	require.NoError(err)
	require.Equal(amount2, unbondTokens)

	// unbond rest of the other delegation's shares
	remainingTokens := delTokens.Sub(unbondTokens)
	_, amount3, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], types.FlexibleDelegationID, math.LegacyNewDecFromInt(remainingTokens))
	require.NoError(err)
...
func (s *KeeperTestSuite) TestUnbondingAllDelegationFromValidator() {
	...
	_, amount2, err := keeper.Undelegate(ctx, addrDels[1], addrVals[0], types.FlexibleDelegationID, math.LegacyNewDecFromInt(delTokens))
	require.NoError(err)
...
func (s *KeeperTestSuite) TestRedelegateToSameValidator() {
	...
	_, err := keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[0], types.FlexibleDelegationID, math.LegacyNewDec(5))
	require.Error(err)
}
...
func (s *KeeperTestSuite) TestRedelegationMaxEntries() {
	...
	for i := uint32(0); i < maxEntries; i++ {
		var err error
		completionTime, err = keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], types.FlexibleDelegationID, math.LegacyNewDec(1))
		require.NoError(err)
	}

	// an additional redelegation should fail due to max entries
	_, err = keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], types.FlexibleDelegationID, math.LegacyNewDec(1))
	require.Error(err)
...
func (s *KeeperTestSuite) TestRedelegateSelfDelegation() {
	...
	_, err := keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], types.FlexibleDelegationID, math.LegacyNewDecFromInt(delTokens))
	require.NoError(err)
...
func (s *KeeperTestSuite) TestRedelegateFromUnbondingValidator() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.NotBondedPoolName, stakingtypes.BondedPoolName, gomock.Any())
	_, err = keeper.BeginRedelegation(ctx, addrDels[1], addrVals[0], addrVals[1], types.FlexibleDelegationID, math.LegacyNewDecFromInt(redelegateTokens))
	require.NoError(err)
...
func (s *KeeperTestSuite) TestRedelegateFromUnbondedValidator() {
	...
	s.bankKeeper.EXPECT().SendCoinsFromModuleToModule(gomock.Any(), stakingtypes.NotBondedPoolName, stakingtypes.BondedPoolName, gomock.Any())
	_, err = keeper.BeginRedelegation(ctx, addrDels[1], addrVals[0], addrVals[1], types.FlexibleDelegationID, math.LegacyNewDecFromInt(redelegationTokens))
	require.NoError(err)
