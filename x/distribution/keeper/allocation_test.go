...
func TestAllocateTokensToManyValidators(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, feePool.UbiPool.IsZero())
	...
	feePool, err = distrKeeper.FeePool.Get(ctx)
	require.NoError(t, err)
	require.Equal(t, sdk.DecCoins{{Denom: sdk.DefaultBondDenom, Amount: math.LegacyNewDec(2)}}, feePool.UbiPool)
...
func TestAllocateTokensTruncation(t *testing.T) {
	...
	val2, err := stakingtypes.NewValidator(sdk.ValAddress(valConsAddr2).String(), valConsPk1, stakingtypes.Description{}, stakingtypes.TokenType_LOCKED)
	require.NoError(t, err)
